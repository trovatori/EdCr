...
// ===== PATCH: TITOLO DALL'HEADER TEI =====
const teiTitle = xml.querySelector('titleStmt > title');
const teiAuthor = xml.querySelector('titleStmt > author');
const headerTitle = document.querySelector('.edition-title');

if (teiTitle && headerTitle) {
  const titleText = teiTitle.textContent.trim();
  const authorText = teiAuthor ? teiAuthor.textContent.trim() : '';
  headerTitle.innerHTML = authorText
  ? `<span class="edition-author">${authorText}, </span><span class="edition-title-text">${titleText}</span>`
  : `<span class="edition-title-text">${titleText}</span>`;
}
// ===== FINE PATCH =====
...
// ===== PATCH: SUPPORTO <seg xml:id="..."> CON NOTE DI COMMENTO =====
// INSERIRE QUESTO BLOCCO COME NUOVO RAMO `else if`
// DENTRO `line.childNodes.forEach(node => { ... })`,
// SUBITO DOPO IL BLOCCO `else if (node.nodeName === 'app') { ... }`
...
// ================= PATCH: TOGGLE INTERNO TRADUZIONE =================

const internalTranslationToggle =
  document.getElementById('translation-toggle-internal');
const navbarTranslationToggle =
  document.getElementById('toggle-translation');

if (internalTranslationToggle && navbarTranslationToggle) {
  internalTranslationToggle.addEventListener('click', () => {
    navbarTranslationToggle.click();
  });
}

// ================= FINE PATCH: TOGGLE INTERNO TRADUZIONE =================
